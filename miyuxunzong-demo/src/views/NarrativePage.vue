...
<script setup lang="ts">
    ...
    const sceneFigure = computed(() => {
        ...
        const figureMap: Record<string, string> = {
            'jiang_nanxiang': '蒋南翔_透明.png',
            'zhang_shoutian': '门卫_透明.png',
            'zhang_huiru': '张同学_透明.png',
            'wu_cuilian': '校工老吴_透明.png',
            'li_wanqing': '室友李_透明.png',
            'chen_xiuzhen': '带孩子的妇女_透明.png',
            'sun_laowu': '老孙_透明.png',
            'female_student': '女学生_透明.png',
            'porter': '脚夫_透明.png',
            'junjing': '军警_透明.png',
            'wo': '我_透明.png'
        ...
        const fileName = figureMap[figure.id]
        if (!fileName) return null

        return {
            name: figure.id,
            position: figure.position ?? 'center',
            src: `${import.meta.env.BASE_URL}assets/figure/${encodeURIComponent(fileName)}`
        }
    })
    ...
    watch(node, value => {
        const speakerMap: Record<string, string> = {
            '蒋南翔': 'jiang_nanxiang', '老吴（校工）': 'wu_cuilian', '老吴': 'wu_cuilian', '张同学': 'zhang_huiru', '室友李': 'li_wanqing', '门卫': 'zhang_shoutian', '女学生': 'female_student',
            '脚夫': 'porter',
            '带孩子的妇女': 'chen_xiuzhen',
            '老孙（便衣特务）': 'sun_laowu',
            '老孙': 'sun_laowu',
...
<style>
    /* 强制清除场景立绘的所有白底、边框、阴影 */
    section.scene > img.scene-figure,
